...
namespace WheelMUD.Ftp.FtpCommands
{
	...
	public class RenameStartCommandHandler : FtpCommandHandler
	{
		...
		protected override string OnProcess(string sMessage)
		{
            ...
            this.ConnectionObject.FileToRename = sFile;

            IFileInfo info = this.ConnectionObject.FileSystemObject.GetFileInfo(sFile);

			if (info == null)
			{
                return this.GetMessage(550, string.Format("File does not exist ({0}).", sFile));
			}

            this.ConnectionObject.RenameDirectory = info.IsDirectory();
            return this.GetMessage(350, string.Format("Rename file started ({0}).", sFile));
		} 
	}
}
