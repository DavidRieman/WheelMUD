    ...
    public class GetNameState : CharacterCreationSubState
    {
        ...
        public override void ProcessInput(string command)
        {
            if (ValidateUserName(ref command))
            {
                ...
                {
                    Session.Write("I'm sorry, that name is already taken. Please choose another.<%nl%>");
                }
        ...
        public override string BuildPrompt()
        {
            return AppConfigInfo.Instance.UserAccountIsPlayerCharacter ?
                "Please enter a name for your character:<%nl%>" :
                "Please enter your user account name:<%nl%>";
        }
        ...
        private bool ValidateUserName(ref string newUserName)
        {
            ...
            {
                Session.Write("You must supply a name.<%nl%>");
                return false;
            }
            ...
            {
                Session.Write($"Player name must be between {MinimumPlayerCharacterNameLength} and {MaximumPlayerCharacterNameLength} letters long. Please choose another.<%nl%>");
                return false;
            }
            ...
            {
                Session.Write($"User name must be between {MinimumUserNameLength} and {MaximumUserNameLength} letters long. Please choose another.<%nl%>");
                return false;
            }
            ...
            if (isAlsoPlayerName)
            {
                ...
                foreach (char c in newUserName.ToCharArray())
                {
                    if (!char.IsLetter(c))
                    {
                        Session.Write("Character name must include only letters. Please choose another.<%nl%>");
                        return false;
                    }
                }
                ...
                if (vowelCount <= 0)
                {
                    Session.Write("Character name may not exclude vowels. Please choose another.<%nl%>");
                    return false;
                }
                ...
                {
                    Session.Write("Character name may not be heavily uppercase. Please choose another.<%nl%>");
                    return false;
                }
