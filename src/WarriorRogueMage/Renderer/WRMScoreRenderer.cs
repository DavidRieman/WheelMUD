...
namespace WheelMUD.Core
{
    ...
    public class DefaultScoreRenderer : RendererDefinitions.Score
    {
        public override string Render(Thing player)
        {
            ...
            sb.AppendAnsiLine("<%green%>+-------------------------------------------------------------------+");
            sb.AppendAnsiLine($"{pipe} {nameAndTitle,-19} Level {"?",-6}  Reputation {"?",-6}  Kudos {"?",-6} {pipe}");
            sb.AppendAnsiLine("<%green%>+----------------+----------------+----------------+----------------+");
            sb.AppendAnsiLine($"{pipe} {healthLine} {pipe} {manaLine} {pipe}");
            sb.AppendAnsiLine($"{pipe} {damageLine} {pipe} {initLine} {pipe}");
            sb.AppendAnsiLine($"{pipe} {attackLine} {pipe} {defenseLine} {pipe}");
            sb.AppendAnsiLine($"{pipe} {armorPenaltyLine} {pipe} {wieldMaxLine} {pipe}");
            sb.AppendAnsiLine($"{pipe} {huntLine} {pipe} {familiarLine} {pipe}");
            sb.AppendAnsiLine($"{pipe} {fateLine} {pipe} {string.Empty.PadRight(31)} {pipe}");
            sb.AppendAnsiLine("<%green%>+---------------------------------^---------------------------------+<%n%>");
